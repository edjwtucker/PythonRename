...
def create_filename(time, ext, tag=None, number=None):
    ...
    """
    tag_text = "" if tag==None else "_" + tag
    if number == None:
        new_filename = time + tag_text + "." + ext
    else:
        new_filename = time + "_" + str(number) + tag_text + "." + ext
    return new_filename


def create_unique_filename(directory, time, ext, tag=None):
    ...
    """
    potential_filename = create_filename(time, ext, tag, None)

    number = 1
    while path.isfile(path.join(directory, potential_filename)):
        potential_filename = create_filename(time, ext, tag, number)
        number += 1
...
def rename_files_by_datetime(directory, tag=None):
    ...
    for filename in listdir_fullpath(directory):
        if path.isfile(filename):
            ...
            new_filename = create_unique_filename(directory, t, ext, tag)
